...
enum json_parse_flags_e {
    ...
    json_parse_flags_allow_simplified_json = (json_parse_flags_allow_trailing_comma
...
json_weak void* json_write_pretty(const struct json_value_s *value,
        const char *indent, const char *newline, size_t *out_size);
    ...
    if ((json_parse_flags_allow_hexadecimal_numbers & flags_bitset)
        ...
        if ((offset < size) && ('.' == src[offset])) {
            ...
            if (!('0' <= src[offset] && src[offset] <= '9')) {
                if (!(json_parse_flags_allow_leading_or_trailing_decimal_point
                        & flags_bitset) || !had_leading_digits) {
        ...
        if ('\\' == src[offset]) {
            ...
            case 'u': {
                ...
                if (codepoint <= 0x7fu) {
                     ...
                     * codepoint. */
                    const unsigned long surrogate_offset = 0x10000u
                            - (0xD800u << 10) - 0xDC00u;
                    codepoint = (high_surrogate << 10) + codepoint
                            + surrogate_offset;
                    high_surrogate = 0;
                    data[bytes_written++] = (char) (0xF0u | (codepoint >> 18)); /* 11110xxx. */
                    data[bytes_written++] = (char) (0x80u
...
json_weak void json_parse_object(struct json_parse_state_s *state,
        int is_global_object, struct json_object_s *object);
...
int json_write_get_number_size(const struct json_number_s *number,
    ...
    cleanup: *size += number->number_size; /* the actual string of the number. */
...
int json_write_minified_get_array_size(const struct json_array_s *array,
    ...
    *size += 2; /* '[' and ']'. */
    ...
    for (element = array->start; json_null != element; element =
            element->next) {
...
int json_write_minified_get_object_size(const struct json_object_s *object,
    ...
    *size += object->length; /* ':'s seperate each name/value pair. */
    ...
    for (element = object->start; json_null != element; element =
            element->next) {
...
char* json_write_pretty_array(const struct json_array_s *array, size_t depth,
        const char *indent, const char *newline, char *data) {
    size_t k, m;
    struct json_array_element_s *element;
